<script lang="ts">
	...
	const totalDebtWithInterest = $derived(
		debts
			.filter((debt) => !debt.is_fully_paid)
	...
	function getDebtInsights() {
		...
		for (const debt of activeDebts) {
			...
			const originalAmount = debt.original_amount
				? parseFloat(debt.original_amount.toString())
				: currentAmount;
		...
		if (activeDebts.length > 0) {
			...
			const highestInterestDebt = [...activeDebts].sort(
				(a, b) => parseFloat(b.interest_rate.toString()) - parseFloat(a.interest_rate.toString())
			)[0];

			// Find debts that can be fully paid with current balance
			const payableDebts = activeDebts
				.filter((debt) => Number(debt.amount) <= currentBalance)
				.sort((a, b) => Number(a.amount) - Number(b.amount)); // Sort by amount, smallest first
	...
	async function handlePayment(event: Event) {
		...
		if (paymentAmount > Number(selectedDebt.amount)) {
	...
	async function handleUpdateDebt(event: Event) {
		...
		if (
			...
			!editFormDescription ||
			!editFormAmount ||
			!editFormInterestRate ||
			!editFormTakenFrom ||
			!editFormStartDate ||
			!editFormExpirationDate
		) {
			error = 'Please fill in all required fields';
			return;
		}
	...
	function getDebtStatus(amount: number, originalAmount: number) {
		if (originalAmount <= 0)
			return { status: 'No Debt', color: 'text-gray-400', bgColor: 'bg-gray-500/20' };
		const percentage = ((originalAmount - amount) / originalAmount) * 100;
		if (percentage >= 100)
			return { status: 'Paid Off', color: 'text-green-400', bgColor: 'bg-green-500/20' };
		if (percentage >= 75)
			return { status: 'Almost Done', color: 'text-yellow-400', bgColor: 'bg-yellow-500/20' };
		if (percentage >= 25)
...
<div class="flex-1 space-y-6 bg-gray-950 p-6">
	...
	{:else}
				...
				<DialogContent class="border-gray-700 bg-gray-800 text-white sm:max-w-[425px]">
					<DialogHeader>
						...
						<DialogDescription class="text-gray-400"
							>Record a new debt to track and manage</DialogDescription
						>
					</DialogHeader>
					...
					<form onsubmit={handleAddDebt} class="space-y-4">
						...
						<div class="space-y-2">
							...
							<select
								bind:value={formType}
								required
								class="w-full rounded border-gray-600 bg-gray-700 px-3 py-2 text-white"
							>
		...
		<div class="grid gap-4 md:grid-cols-3">
			...
			<Card class="border-gray-800 bg-gray-900">
				...
				<CardContent>
					<div class="text-2xl font-bold text-red-600">
						${(totalDebtWithInterest || 0).toFixed(2)}
					</div>
					<p class="text-xs text-gray-400">
						{debts.filter((debt) => !debt.is_fully_paid).length} active of {debts.length} total debts
					</p>
				</CardContent>
			</Card>
			...
			<Card class="border-gray-800 bg-gray-900">
				...
				<CardContent>
					...
					<p class="text-xs text-gray-400">
						{typeof earliestDeadline === 'string'
							? 'No active debts'
							: earliestDeadline.description}
					</p>
				</CardContent>
			</Card>
		</div>
			...
			<CardContent>
				<div class="space-y-6">
					{#each paginatedData.debts as debt (debt.id)}
						{@const originalDebtAmount = debt.original_amount
							? parseFloat(debt.original_amount.toString())
							: Number(debt.amount) || 0}
						{@const currentAmount = Number(debt.amount) || 0}
						{@const paidAmount = originalDebtAmount - currentAmount}
						{@const debtStatus = getDebtStatus(Number(debt.amount) || 0, originalDebtAmount)}
						{@const timeRemaining = getRemainingTime(debt)}

						<div class="relative rounded-lg border border-gray-700 bg-gray-800 p-4">
								...
								<div class="flex items-baseline gap-1">
									...
									{:else}
										...
										<span class="text-lg font-bold text-white"
											>${(Number(debt.amount) || 0).toFixed(2)}</span
										>
									{/if}
								</div>
							</div>
							...
							<div class="absolute right-2 top-20 flex gap-2">
								{#if !debt.is_fully_paid}
									<Button
										variant="outline"
										size="sm"
										...
										class="border-2 border-blue-500 bg-gray-900 font-semibold text-blue-400 shadow-lg hover:bg-blue-500/20 hover:text-blue-300"
									>
										Pay Amount
									</Button>
								{/if}
							...
							<div class="pr-4">
								<div>
									<div class="flex items-center gap-2">
										...
										<Badge variant={debt.type === 'bank' ? 'default' : 'secondary'}
											>{debt.type}</Badge
										>
										...
										{#if debt.is_fully_paid}
											<Badge class="border-0 bg-green-500/20 text-green-400">Fully Paid</Badge>
										{/if}
									</div>
									<p class="text-sm text-gray-400">
										{debt.interest_rate}% Interest Rate • Due: {new Date(
											debt.expiration_date
										).toLocaleDateString()}
									</p>
									...
									<div class="mt-2">
										<span
											class="text-base font-medium {timeRemaining.isFullyPaid
												? 'text-green-400'
		...
		<Card class="border-gray-800 bg-gray-900">
			<CardHeader>
				...
				<CardDescription class="text-gray-400"
					>AI-powered recommendations for your debt management</CardDescription
				>
			</CardHeader>
			<CardContent>
				<div class="space-y-4">
					...
					{#if debtInsights.alert}
						{#if debtInsights.alert.type === 'payment_behind_schedule' && debtInsights.alert.progressGap !== undefined && debtInsights.alert.timeElapsed !== undefined && debtInsights.alert.paymentProgress !== undefined}
							<div class="rounded-lg border border-red-500/20 bg-red-500/10 p-4">
								<h4 class="font-semibold text-red-400">Payment Behind Schedule</h4>
								<p class="mt-1 text-sm text-gray-300">
									Your {debtInsights.alert.debt.description} debt is {debtInsights.alert.progressGap.toFixed(
										0
									)}% behind schedule.
									{debtInsights.alert.timeElapsed.toFixed(0)}% of time has passed but only {debtInsights.alert.paymentProgress.toFixed(
										0
									)}% has been paid. Consider increasing your payment amount to stay on track.
								</p>
							</div>
						{:else if debtInsights.alert.type === 'deadline_approaching' && debtInsights.alert.currentAmount !== undefined}
							<div class="rounded-lg border border-yellow-500/20 bg-yellow-500/10 p-4">
								...
								<p class="mt-1 text-sm text-gray-300">
									Your {debtInsights.alert.debt.description} debt is due in {debtInsights.alert
										.daysRemaining} day{debtInsights.alert.daysRemaining !== 1 ? 's' : ''}! You
									still owe ${debtInsights.alert.currentAmount.toFixed(2)}. Take immediate action to
									avoid penalties.
								</p>
							</div>
						{:else if debtInsights.alert.type === 'debt_exceeds_balance' && debtInsights.alert.currentAmount !== undefined && debtInsights.alert.ratio !== undefined && debtInsights.alert.currentBalance !== undefined}
							<div class="rounded-lg border border-red-500/20 bg-red-500/10 p-4">
								<h4 class="font-semibold text-red-400">Debt Significantly Exceeds Balance</h4>
								<p class="mt-1 text-sm text-gray-300">
									Your {debtInsights.alert.debt.description} debt (${debtInsights.alert.currentAmount.toFixed(
										2
									)}) is {debtInsights.alert.ratio.toFixed(1)}x larger than your current balance (${debtInsights.alert.currentBalance.toFixed(
										2
					...
					{#if debtInsights.progress}
						<div class="rounded-lg border border-green-500/20 bg-green-500/10 p-4">
							...
							<p class="mt-1 text-sm text-gray-300">
								Outstanding work on your {debtInsights.progress.debt.description} debt! You're {debtInsights.progress.progressAdvantage.toFixed(
									0
								)}% ahead of schedule. You've paid {debtInsights.progress.paymentProgress.toFixed(
									0
								)}% while only {debtInsights.progress.timeElapsed.toFixed(0)}% of the time period
								has passed.
							</p>
						</div>
					{/if}
					...
					<!-- Tip Box (Blue) -->
					{#if debtInsights.tip}
						{#if debtInsights.tip.type === 'payoff_opportunity' && debtInsights.tip.debt && debtInsights.tip.currentBalance !== undefined}
							<div class="rounded-lg border border-blue-500/20 bg-blue-500/10 p-4">
								...
								<p class="mt-1 text-sm text-gray-300">
									You can fully pay off your {debtInsights.tip.debt.description} debt (${Number(
										debtInsights.tip.debt.amount
									).toFixed(2)}) with your current balance (${debtInsights.tip.currentBalance.toFixed(
										2
									)}). Paying off smaller debts first can provide psychological momentum and reduce
									your total debt count.
								</p>
							</div>
						{:else if debtInsights.tip.type === 'prioritize_high_interest' && debtInsights.tip.debt && debtInsights.tip.interestRate !== undefined}
							<div class="rounded-lg border border-blue-500/20 bg-blue-500/10 p-4">
								<h4 class="font-semibold text-blue-400">Focus on High Interest</h4>
								<p class="mt-1 text-sm text-gray-300">
									Consider prioritizing payments on your {debtInsights.tip.debt.description} debt with
									{debtInsights.tip.interestRate.toFixed(1)}% interest rate. Higher interest debts
									cost more over time, so paying them faster saves money in the long run.
								</p>
							</div>
						{:else if debtInsights.tip.type === 'increase_balance' && debtInsights.tip.debtCount !== undefined}
							<div class="rounded-lg border border-blue-500/20 bg-blue-500/10 p-4">
								<h4 class="font-semibold text-blue-400">Increase Available Balance</h4>
								<p class="mt-1 text-sm text-gray-300">
									With {debtInsights.tip.debtCount} active debt{debtInsights.tip.debtCount > 1
										? 's'
										: ''} and limited balance, consider increasing your income or reducing expenses to
									have more funds available for debt payments.
								</p>
							</div>
						{/if}
					{:else}
						<div class="rounded-lg border border-blue-500/20 bg-blue-500/10 p-4">
							...
							<p class="mt-1 text-sm text-gray-300">
								Consider the debt avalanche method: pay minimums on all debts, then put extra money
								toward the highest interest rate debt first to minimize total interest paid.
							</p>
						</div>
					{/if}
				</div>
		...
		<Dialog bind:open={isPaymentOpen}>
			<DialogContent class="border-gray-700 bg-gray-800 text-white">
				<DialogHeader>
					<DialogTitle>Make Payment - {selectedDebt?.description}</DialogTitle>
					<DialogDescription class="text-gray-400">
						{#if selectedDebt}
							Current debt balance: ${(Number(selectedDebt.amount) || 0).toFixed(2)}
							<br />
							{#if balance && balance.balance !== null && balance.balance !== undefined}
				...
				<form onsubmit={handlePayment} class="space-y-4">
					<div class="space-y-2">
						...
						<Input
							...
							type="number"
							step="0.01"
							placeholder={selectedDebt ? (Number(selectedDebt.amount) * 0.02).toFixed(2) : '0.00'}
							min={0}
							max={selectedDebt ? Number(selectedDebt.amount) : undefined}
							required
							class="border-gray-600 bg-gray-700"
						/>
						{#if balance && balance.balance !== null && balance.balance !== undefined}
							...
							{:else if selectedDebt && availableBalance < Number(selectedDebt.amount)}
								<p class="text-sm text-yellow-400">
									⚠️ Balance lower than total due amount. Max payment: ${availableBalance.toFixed(
										2
									)}
								</p>
							{/if}
							...
							{#if selectedDebt}
								{@const availableBalance =
									balance && balance.balance !== null && balance.balance !== undefined
										? parseFloat(balance.balance.toString())
										: 0}
								{@const debtAmount = Number(selectedDebt.amount)}
								...
								<Button
									...
									class="border-gray-600 bg-transparent text-gray-300 transition-all hover:bg-gray-600 hover:text-white disabled:opacity-50"
								>
									$10
								</Button>
								...
								<Button
									...
									class="col-span-2 border-green-500 bg-transparent text-green-300 transition-all hover:bg-green-500 hover:text-white disabled:opacity-50"
								>
					...
					<div class="flex gap-2">
						<Button
							...
							class="flex-1 border-gray-600 bg-gray-800 text-white hover:bg-gray-700"
						>
							Cancel
						</Button>
						<Button
