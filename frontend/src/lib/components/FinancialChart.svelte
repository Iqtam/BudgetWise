<script lang="ts">
	...
	export let data: { month: string; income: number; expenses: number }[] = [];
	...
	$: maxValue = chartData.length > 0 ? Math.max(...chartData.map(d => d.income)) : 0;
	$: minValue = 0;
	...
	function getChartPoints() {
		if (chartData.length === 0) return { incomePoints: [], expensePoints: [] };
	...
	function handleMouseMove(event: MouseEvent) {
		...
		const stepX = 100 / (chartData.length - 1);
			...
			{:else}
				...
				<div class="w-full h-full flex items-center justify-center">
					<div class="text-center">
						<div class="text-gray-400 text-sm mb-2">{monthCount === 0 ? "No transaction data available" : 
				  monthCount === 1 ? "Your spending pattern this month" :
				  `Your spending patterns over the last ${monthCount} months`}</div>
						<div class="text-gray-500 text-xs">Add some transactions to see your spending overview</div>
					</div>
				</div>
			{/if}
		</div>
