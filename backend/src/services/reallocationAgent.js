...
class ReallocationAgent {
  ...
  async analyzeAndRecommendReallocation(userId, timeframe = 'monthly', targetGoals = []) {
    try {
      console.log(`Starting reallocation analysis for user: ${userId}`);
      
      // Phase 1: Gather comprehensive financial data
      const financialData = await this.gatherReallocationData(userId, timeframe);
      
      // Phase 2: Analyze spending vs budget variances
      const varianceAnalysis = await this.analyzeSpendingVariances(financialData);
      
      // Phase 3: Identify reallocation opportunities
      const opportunities = await this.identifyReallocationOpportunities(varianceAnalysis, targetGoals);
      
      // Phase 4: Generate specific reallocation recommendations
      const recommendations = await this.generateReallocationRecommendations(opportunities, financialData);
      
      // Phase 5: Calculate impact projections
      const projections = await this.calculateReallocationImpact(recommendations, financialData);
      
      // Phase 6: Create actionable reallocation plan
      const plan = await this.createReallocationPlan(recommendations, projections);
  ...
  async analyzeSpendingVariances(financialData) {
    ...
    const untrackedSpending = this.analyzeUntrackedSpending(spendingByCategory, currentBudgets, categories);
    
    return {
      categoryVariances: categoryVariances.sort((a, b) => Math.abs(b.variance) - Math.abs(a.variance)),
      totalBudgeted,
      totalSpent,
      totalVariance: totalSpent - totalBudgeted,
      totalVariancePercent: totalBudgeted > 0 ? ((totalSpent - totalBudgeted) / totalBudgeted) * 100 : 0,
      untrackedSpending,
      ...
      highVarianceCategories: categoryVariances.filter(v => Math.abs(v.variancePercent) > 20)
    };
  }
  ...
  async identifyReallocationOpportunities(varianceAnalysis, targetGoals) {
    ...
    varianceAnalysis.overspendingCategories.forEach(category => {
      if (category.variancePercent > 15) {
        opportunities.push({
          ...
          reason: `Consistently overspending by ${category.variancePercent.toFixed(1)}%`,
    ...
    varianceAnalysis.underspendingCategories.forEach(category => {
      if (category.variancePercent < -25 && category.actualAmount > 0) {
        opportunities.push({
          ...
          reason: `Underutilizing budget by ${Math.abs(category.variancePercent).toFixed(1)}%`,
          priority: 'medium',
          impact: 'medium'
        });
      }
    ...
    if (targetGoals.length > 0) {
      const goalOpportunities = this.identifyGoalBasedOpportunities(varianceAnalysis, targetGoals);
    ...
    if (varianceAnalysis.untrackedSpending.total > 100) {
      opportunities.push({
        ...
        suggestedCategories: varianceAnalysis.untrackedSpending.categories.slice(0, 3),
        priority: 'high',
        impact: 'high'
      });
    }
  ...
  async generateReallocationRecommendations(opportunities, financialData) {
    ...
    opportunities.forEach(opportunity => {
      switch (opportunity.type) {
        case this.reallocationStrategies.OVERSPENDING_ADJUSTMENT:
          recommendations.push({
            id: `realloc_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            type: opportunity.type,
            title: `Increase ${opportunity.categoryName} Budget`,
            description: `Increase budget from $${opportunity.currentBudget.toFixed(0)} to $${opportunity.suggestedBudget.toFixed(0)}`,
            fromCategories: this.findReallocationSources(opportunity.suggestedBudget - opportunity.currentBudget, financialData),
            toCategory: {
        ...
        case this.reallocationStrategies.UNDERUTILIZATION_OPTIMIZATION:
          recommendations.push({
            ...
            description: `Reduce budget from $${opportunity.currentBudget.toFixed(0)} to $${opportunity.suggestedBudget.toFixed(0)}`,
            fromCategory: {
              id: opportunity.categoryId,
              name: opportunity.categoryName,
              currentBudget: opportunity.currentBudget,
            ...
            suggestedTargets: this.suggestReallocationTargets(opportunity.availableReallocation, financialData),
            reason: opportunity.reason,
            priority: opportunity.priority,
            expectedImpact: this.calculateExpectedImpact(opportunity),
            timeline: 'next_cycle'
      ...
      totalReallocation += opportunity.suggestedBudget ? Math.abs(opportunity.suggestedBudget - opportunity.currentBudget) : 0;
    });
    ...
    return {
      ...
      estimatedTimeToImplement: this.estimateImplementationTime(recommendations)
    };
  }
    ...
    reallocs.forEach(rec => {
      switch (rec.type) {
        ...
        case this.reallocationStrategies.UNDERUTILIZATION_OPTIMIZATION:
          ...
          budgetEfficiencyImprovement += 0.10; // 10% efficiency gain
          break;
      }
    });
    
    return {
      budgetEfficiencyImprovement: Math.min(budgetEfficiencyImprovement, 0.50), // Cap at 50%
      overspendingReduction,
      availableFundsIncrease,
      projectedMonthlySavings: overspendingReduction + (availableFundsIncrease * 0.7),
  ...
  async createReallocationPlan(recommendations, projections) {
    ...
    const nextCycleActions = reallocs.filter(r => r.timeline === 'next_cycle');
    ...
    const plan = {
      immediate: {
        ...
        totalImpact: immediateActions.reduce((sum, a) => sum + (a.amount || 0), 0)
      },
      ...
      nextCycle: {
        ...
        totalImpact: nextCycleActions.reduce((sum, a) => sum + (a.availableAmount || a.amount || 0), 0)
      },
      ...
      summary: {
        ...
        confidenceLevel: projections.confidenceScore > 0.8 ? 'high' : projections.confidenceScore > 0.6 ? 'medium' : 'low'
      }
    };
  ...
  analyzeUntrackedSpending(spendingByCategory, budgets, categories) {
    ...
    Object.entries(spendingByCategory).forEach(([categoryId, amount]) => {
      if (!budgetedCategories.has(categoryId) && categoryId !== 'uncategorized') {
        const category = categories.find(c => c.id === categoryId);
        untracked.push({
          categoryId,
          categoryName: category?.name || 'Unknown',
  ...
  suggestReallocationTargets(availableAmount, financialData) {
    ...
    const targets = [
      { type: 'emergency_fund', priority: 'high', suggestedAmount: availableAmount * 0.4 },
      { type: 'debt_payment', priority: 'high', suggestedAmount: availableAmount * 0.3 },
      { type: 'savings_goal', priority: 'medium', suggestedAmount: availableAmount * 0.3 }
    ];
  ...
  calculateExpectedImpact(opportunity) {
    ...
    return {
      budgetAccuracy: opportunity.type === this.reallocationStrategies.OVERSPENDING_ADJUSTMENT ? 0.25 : 0.15,
  ...
  async storeReallocationResults(userId, results) {
    try {
      ...
      console.log(`Stored reallocation results for user ${userId}: ${reallocationRecord.id}`);
